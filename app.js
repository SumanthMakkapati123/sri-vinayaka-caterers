...
async function load(){try{
 ...
 $('#contact-details').innerHTML=`<a href="tel:${e(catalog.business.phone)}">${e(catalog.business.phone)} (WhatsApp & Call)</a><a href="tel:${e(catalog.business.alternatePhone)}">${e(catalog.business.alternatePhone)} (Call)</a><a class="email" href="mailto:${e(catalog.business.email)}">${e(catalog.business.email)}</a><address>${e(catalog.business.address)}</address>`;
 ...
 }catch(err){$('#catalog-status').innerHTML=`${e(err.message)} <a href="tel:+919533032737">Call +91 95330 32737</a> or <button class="text-link" id="retry-load">try again</button>.`;}}
